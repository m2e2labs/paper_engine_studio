...
Six commands for writing a book, and five for shipping one. Every one takes paths relative to the repo root, and every one carries a
...
| `build.mjs` | **Use this one.** Runs `build-book.mjs`, then adds the front and back matter listed in `book.json` under `matter` (dedication, preface, sources, about the author) and renumbers the contents and index from where every sheet ended up. Same flags. With no `matter`, the output is identical. | `node engine/tools/build.mjs books/<slug>` |
