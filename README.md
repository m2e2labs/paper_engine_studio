...
node engine/tools/preflight.mjs books/showcase                  # 22 checks between "it builds" and "it ships"
...
- **Preflight** is the release gate: every picture's source and rights on record, `book.json` valid, the store listing complete, every
  ...
  approved every page.
- **Screenshots**, for books about software: a band that never crops the capture, numbered
  pins and marks placed in percent, a capture tool, and a preflight check that measures
  whether the text in the capture can still be read at its printed size.
